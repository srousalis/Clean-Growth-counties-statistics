...
#%% load data
stations_path = os.path.join(directory_path, "input", "vertices.csv")
df_stations = pd.read_csv(stations_path)

lines_path = os.path.join(directory_path, "input", "links_cap.csv")
df_lines = pd.read_csv(lines_path)

intersections_path = os.path.join(directory_path, "input", "intersections.csv")
...
#%% Calculate the number of lines within and crossing counties for each capacity bin

# Group by capacity index and within/cross county status
lines_by_capacity_bin = (
    final_edges
    .groupby(['capacity_index', 'GEOID'])
    .agg({
        'within_county': 'sum',  # Sum of within-county lines
        'cross_county': 'sum'    # Sum of cross-county lines
    })
    .reset_index()
)

# Pivot the table to create a summary per county for each capacity bin
lines_by_capacity_bin_pivot = lines_by_capacity_bin.pivot_table(
    index='GEOID',
    columns='capacity_index',
    values=['within_county', 'cross_county'],
    aggfunc='sum',
    fill_value=0
)

# Flatten the multi-level columns from pivot_table
lines_by_capacity_bin_pivot.columns = [
    f'{col[0]}_bin_{col[1]}' for col in lines_by_capacity_bin_pivot.columns
]

# Merge the results back with the county stats to get full county information
full_county_stats_with_capacity = pd.merge(
    full_county_stats, 
    lines_by_capacity_bin_pivot, 
    left_on='county_geo_id', 
    right_index=True, 
    how='left'
)

# Fill NaN values with 0 for counties with no data
full_county_stats_with_capacity.fillna(0, inplace=True)

columns_order = [
    'county_name', 'county_geo_id',
    'lines_count_within_county','lines_count_cross_county',
    'lines_distance_km_max_cap_mw_1000',  'within_county_bin_1','cross_county_bin_1',
    'lines_distance_km_max_cap_mw_2000',  'within_county_bin_2','cross_county_bin_2',
    'lines_distance_km_max_cap_mw_3000',  'within_county_bin_3','cross_county_bin_3',
    'lines_distance_km_max_cap_mw_4000', 'within_county_bin_4','cross_county_bin_4',
    'lines_distance_km_max_cap_mw_5000', 'within_county_bin_5','cross_county_bin_5'
]

full_county_stats_with_capacity = full_county_stats_with_capacity[columns_order]

full_county_stats_with_capacity = full_county_stats_with_capacity.rename(columns={
    'within_county_bin_1': 'lines_count_within_county_max_cap_mw_1000',
    'within_county_bin_2': 'lines_count_within_county_max_cap_mw_2000',
    'within_county_bin_3': 'lines_count_within_county_max_cap_mw_3000',
    'within_county_bin_4': 'lines_count_within_county_max_cap_mw_4000',
    'within_county_bin_5': 'lines_count_within_county_max_cap_mw_5000',
    
    'cross_county_bin_1': 'lines_count_cross_county_max_cap_mw_1000',
    'cross_county_bin_2': 'lines_count_cross_county_max_cap_mw_2000',
    'cross_county_bin_3': 'lines_count_cross_county_max_cap_mw_3000',
    'cross_county_bin_4': 'lines_count_cross_county_max_cap_mw_4000',
    'cross_county_bin_5': 'lines_count_cross_county_max_cap_mw_5000'
    
    })

# Save the results
full_county_stats_with_capacity.to_excel(os.path.join(directory_path, "output","counties_lines_distance_by_capacity_bin.xlsx"),index=False)
